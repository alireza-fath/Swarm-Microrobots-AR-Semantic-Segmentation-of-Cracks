...
def train(opt, model, device):
    ...
    # optimizer = torch.optim.RMSprop(model.parameters(), lr=opt.lr, momentum=0.9, foreach=True)
    ...
    train_data = CrackAugment(root=os.path.join(opt.data, 'train'), image_size=opt.image_size, suffix_images="rgb", suffix_masks="BW")
    test_data = CrackAugment(root=os.path.join(opt.data, 'train'), image_size=opt.image_size, suffix_images="rgb", suffix_masks="BW")
    ...
    train_loader = DataLoader(train_data, batch_size=opt.batch_size, num_workers=4, shuffle=True, pin_memory=True)
    test_loader = DataLoader(test_data, batch_size=opt.batch_size, num_workers=4, drop_last=True, pin_memory=True)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        logging.info(f"TRAINING: Dice Loss: {(epoch_loss / len(train_loader)):.4f}")
